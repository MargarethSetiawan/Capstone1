...
def index():
    ...
    stats= {
        ...
        'rev_table' : df2[["Category","App","Reviews","Rating"]].groupby(["Category","App"]).agg({"Reviews":"sum","Rating":"mean"}).sort_values(['Reviews','Category'],ascending=False).groupby(['Category', 'Reviews']).head(5).reset_index().head(10).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    df2.pivot_table(index='Category',
                values='Reviews').sort_values(by='Reviews', ascending=False).head().plot.bar()
